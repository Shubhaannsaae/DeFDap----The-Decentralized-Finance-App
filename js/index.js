...
async function loadInitialData(sClass) {
    ...
    try{
        ...
        let stakeDays = await cObj.methods.getStaketDays().call();

        let days = Math.floor(Number(stakeDays) / (3600 * 24))

        let dayDispaly = days > 0 ? days + (days == 1 ? " day, " : " days, ") : "";

        document
                .querySelectorAll(".Lock-period-value")
                .forEach(function(element) {
                    element.innerHTML = `${dayDispaly}`;
                });
...
async function stackTokens() {
    try{
        ...
        let balMainAllownace = await oContractToken.methods
            .allowance(
                currentAddress,
                SELECT_CONTRACT[_NETWORK_ID].STACKING[sClass].address
            )
        ...
        if(Number(balMainAllownace) < Number(tokenToTransfer)) {
